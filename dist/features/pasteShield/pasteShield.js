...
exports.registerPasteShield = registerPasteShield;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const patternDetector_1 = require("./patternDetector");
const pasteShieldConfig_1 = require("./pasteShieldConfig");
...
const ignorePatternsManager_1 = require("./ignorePatternsManager");
const logger = (0, logger_1.createLogger)('PasteShield');
// ─── Constants ───────────────────────────────────────────────────────────────
 ...
 */
function isPasteExcludedFile(document) {
...
function createDecorationTypes() {
    const make = (themeColor, glyph) => vscode.window.createTextEditorDecorationType({
        after: {
 ...
 */
class PasteShieldCodeLensProvider {
    constructor() {
    ...
    provideCodeLenses(document, _token) {
        const config = (0, pasteShieldConfig_1.getConfig)();
        if (!config.enabled) {
            return [];
        }
        const text = document.getText();
        const detections = (0, patternDetector_1.scanContent)(text, {
            ignoredPatterns: config.ignoredPatterns,
        });
        const filtered = detections.filter(d => (0, pasteShieldConfig_1.meetsSeverityThreshold)(d.severity, config.minimumSeverity));
        const lenses = [];
...
let historyManager;
let historyViewProvider;
let historyTreeView;
let statisticsManager;
let ignorePatternsManager;
/**
 ...
 */
const debouncedPruneDecorations = (0, debounce_1.debounce)(pruneStaleDecorations, 150);
/**
 * Debounced CodeLens refresh — avoids hammering the provider on every keystroke.
 */
const debouncedRefreshCodeLens = (0, debounce_1.debounce)(() => {
    codeLensProvider?.refresh();
}, 500);
// ─── Activation ───────────────────────────────────────────────────────────────
function registerPasteShield(context) {
    decorationTypes = createDecorationTypes();
    codeLensProvider = new PasteShieldCodeLensProvider();
    // Initialize History Manager
    historyManager = historyManager_1.HistoryManager.getInstance(context);
    historyViewProvider = new historyViewProvider_1.HistoryViewProvider(historyManager);
    // Initialize Statistics Manager
    statisticsManager = statisticsManager_1.StatisticsManager.getInstance(historyManager);
    // Initialize Ignore Patterns Manager
    ignorePatternsManager = ignorePatternsManager_1.IgnorePatternsManager.getInstance(context);
    ...
    context.subscriptions.push(vscode.languages.registerCodeLensProvider({ scheme: 'file' }, // all files
    codeLensProvider));
    // CodeLens commands
    context.subscriptions.push(vscode.commands.registerCommand(CODELENS_DETAILS_COMMAND_ID, async (detections) => {
        lastReport = detections;
        await showDetailsPanel(detections);
    }));
    context.subscriptions.push(vscode.commands.registerCommand(CODELENS_IGNORE_COMMAND_ID, async (patternName) => {
        await addToIgnoredPatterns(patternName);
        codeLensProvider?.refresh();
    }));
    context.subscriptions.push(vscode.commands.registerCommand(CODELENS_FIX_COMMAND_ID, async (_patternName) => {
        // Placeholder: open settings or guide user to rotate the credential
        await vscode.commands.executeCommand('workbench.action.openSettings', 'pasteShield.ignoredPatterns');
    }));
    // Show detection details from history view
    context.subscriptions.push(vscode.commands.registerCommand(SHOW_DETECTION_DETAILS_COMMAND_ID, async (detection) => {
        await vscode.window.showInformationMessage(`Detection Details\nType: ${detection.type}\nSeverity: ${detection.severity.toUpperCase()}${detection.category ? `\nCategory: ${detection.category}` : ''}`, { modal: true });
    }));
    // ── Paste intercept command ────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand(COMMAND_ID, handlePaste));
    // Toggle on/off from status bar or command palette
    context.subscriptions.push(vscode.commands.registerCommand(TOGGLE_COMMAND_ID, togglePasteShield));
    // Show last scan report
    context.subscriptions.push(vscode.commands.registerCommand(SHOW_REPORT_COMMAND_ID, showLastReport));
    // ── History commands ───────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand(SHOW_HISTORY_COMMAND_ID, () => {
        if (historyTreeView) {
            historyTreeView.reveal(undefined, { focus: true, select: true });
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand(CLEAR_HISTORY_COMMAND_ID, async () => {
        const confirm = await vscode.window.showWarningMessage('Are you sure you want to clear all scan history?', { modal: true }, 'Clear');
    ...
    context.subscriptions.push(vscode.commands.registerCommand(EXPORT_HISTORY_JSON_COMMAND_ID, async () => {
        if (!historyManager)
            return;
        await exportHistory('json');
    }));
    context.subscriptions.push(vscode.commands.registerCommand(EXPORT_HISTORY_TEXT_COMMAND_ID, async () => {
        if (!historyManager)
            return;
        await exportHistory('text');
    }));
    context.subscriptions.push(vscode.commands.registerCommand(REFRESH_HISTORY_COMMAND_ID, () => {
        historyViewProvider?.refresh();
    }));
    // Statistics Dashboard command
    context.subscriptions.push(vscode.commands.registerCommand(SHOW_STATISTICS_COMMAND_ID, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(EXPORT_AUDIT_LOG_COMMAND_ID, async () => {
        if (!historyManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(SHOW_ROTATION_REMINDERS_COMMAND_ID, async () => {
        await showRotationReminders();
    }));
    // Add to Workspace Ignore command
    context.subscriptions.push(vscode.commands.registerCommand(ADD_TO_WORKSPACE_IGNORE_COMMAND_ID, async (pattern) => {
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration(pasteShieldConfig_1.CONFIG_SECTION)) {
            updateStatusBar((0, pasteShieldConfig_1.getConfig)().enabled);
            codeLensProvider?.refresh();
            historyViewProvider?.refresh();
            ignorePatternsManager?.refresh();
        }
...
async function handlePaste() {
    ...
    if (isPasteExcludedFile(editor.document)) {
        logger.info(`PasteShield: paste interception skipped for excluded file "${path.basename(editor.document.fileName)}"`);
        await executeFallbackPaste();
        return;
    }
    ...
    const choice = await vscode.window.showWarningMessage(`PasteShield: ${summary}`, 'Paste Anyway', 'Show Details', 'Cancel');
    ...
    if (choice === 'Show Details') {
        ...
        const finalChoice = await vscode.window.showWarningMessage(`PasteShield — ${filtered.length} issue(s) found. Paste anyway?`, { modal: true }, 'Paste Anyway');
 ...
 */
function runScanAsync(text, options) {
    return new Promise(resolve => {
        // setTimeout(0) yields to allow any pending UI updates first
        setTimeout(() => {
            const start = performance.now();
            const results = (0, patternDetector_1.scanContent)(text, options);
 ...
 */
async function executeFallbackPaste() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
...
async function insertText(editor, text) {
    ...
    if (!success) {
        ...
        vscode.window.showErrorMessage('PasteShield: Could not paste — document may be read-only.');
    }
    return firstRange;
}
/**
 ...
 */
function buildSummaryMessage(detections) {
 ...
 */
async function showDetailsPanel(detections) {
    const lines = [
 ...
 */
async function addToIgnoredPatterns(patternName) {
    const config = vscode.workspace.getConfiguration(pasteShieldConfig_1.CONFIG_SECTION);
    const current = config.get('ignoredPatterns', []);
    if (current.includes(patternName)) {
        vscode.window.showInformationMessage(`PasteShield: "${patternName}" is already ignored.`);
        return;
    }
    await config.update('ignoredPatterns', [...current, patternName], vscode.ConfigurationTarget.Global);
    vscode.window.showInformationMessage(`PasteShield: Pattern "${patternName}" added to ignored list.`);
}
/**
 * Applies a short-lived inline decoration at `range` in `editor`.
 * Auto-clears after 10 seconds.
 */
function applyDecoration(editor, range, severity) {
    if (!decorationTypes) {
        return;
    }
    ...
    setTimeout(() => {
        ...
        activeDecorations = activeDecorations.filter(d => !(d.editor === editor && d.type === dt));
    }, 10000);
}
/**
 * Removes decorations for editors that are no longer visible.
 */
function pruneStaleDecorations() {
...
async function togglePasteShield() {
    const config = vscode.workspace.getConfiguration(pasteShieldConfig_1.CONFIG_SECTION);
...
async function exportAuditLog() {
    ...
    const enableAuditLogging = config.get('enableAuditLogging', true);
 ...
 */
async function showRotationReminders() {
    if (!historyManager)
        return;
    const config = vscode.workspace.getConfiguration('pasteShield');
    const reminderDays = config.get('secretRotationReminderDays', 90);
    const history = historyManager.getHistory();
    const now = Date.now();
    const reminderThreshold = reminderDays * 24 * 60 * 60 * 1000;
    // Group detections by type and find the oldest occurrence
    const oldestDetections = {};
    ...
    if (needsRotation.length === 0) {
        vscode.window.showInformationMessage(`🎉 All detected secrets have been rotated recently!\\n\\nNo secrets older than ${reminderDays} days found.`);
        return;
    }
    ...
    const message = `⚠️ Secret Rotation Reminders\\n\\n` +
        ...
        needsRotation.slice(0, 10).map(s => `• ${s.type}${s.category ? ` (${s.category})` : ''} - ${s.daysSinceDetection} days ago [${s.severity.toUpperCase()}]`).join('\\n') +
...
//# sourceMappingURL=pasteShield.js.map